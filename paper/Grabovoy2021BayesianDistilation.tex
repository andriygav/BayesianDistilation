...
\label{eq:st:5}
\begin{aligned}
g = \bm{\sigma} \circ \mathbf{W}_L\bm{\sigma}  \circ \cdots \circ \mathbf{W}_1, \quad \mathbf{W}_l \in \mathbb{R}^{n_s \times n_{s-1}},
\end{aligned}
\end{gather}
...
\end{aligned}
\end{gather}
вектора~$\mathbf{u},$ которое описывает удаление одного нейрона из~$t$-го слояучителя. Другими словами, преобразование~$\phi\bigr(t\bigr)$ зануляет одну из строк матрицы~$\mathbf{U}_t$. Заметим, что данное зануление влечет зануление соответствующего столбца матрицы~$\mathbf{U}_{t+1}$.

Обозначим новый вектор параметров~$\mathbf{u}^{\phi} =  \phi\bigr(t, \mathbf{u}\bigr),$ а элементы вектора, которые были удалены как~$\mathbf{u}^{\bar{\phi}}.$ Заметим, что векторы~$\mathbf{u}^{\phi} $ и $\mathbf{u}^{\bar{\phi}}$ являются случайными величинами. Введем следующие обозначения:
\begin{gather}
\label{eq:ap:def:1}
\begin{aligned}
\mathbf{u}_{0}^{\phi} = \mathsf{E}\mathbf{u}^{\phi}, \quad \mathbf{u}_{0}^{\bar{\phi}} = \mathsf{E}\mathbf{u}^{\bar{\phi}}, \quad  \bm{\Sigma}_0^{i,j} = \text{cov}\bigr[\mathbf{u}^{i},\mathbf{u}^{j}\bigr],\\
\end{aligned}
\end{gather}
где индексы~$i,j$ принимают значения~$\phi, \bar{\phi}.$
\begin{theorem}
...
\end{enumlist}
Тогда апостериорное распределения параметров модели учителя~$p\bigr(\mathbf{u}^{\phi}|\mathfrak{D}\bigr)$ является нормальным распределением со следующей плотностью распределения:
\begin{gather}
\label{eq:ap:3}
\begin{aligned}
p\bigr(\mathbf{u}^{\phi}|\mathfrak{D}\bigr) = \mathcal{N}\bigr(\mathbf{u}_{0}^{\phi}+\bm{\Sigma}_0^{\phi, \bar{\phi}}\bigr({\bm{\Sigma}_0^{\bar{\phi},\bar{\phi}}}\bigr)^{-1}\left(\mathbf{0} - \mathbf{u}_0^{\bar{\phi}}\right), \bm{\Sigma}_0^{\phi,\phi}-\bm{\Sigma}_0^{\phi, \bar{\phi}}\bigr({\bm{\Sigma}_0^{\bar{\phi}, \bar{\phi}}}\bigr)^{-1}\bm{\Sigma}_0^{\phi, \bar{\phi}}\bigr),
\end{aligned}
\end{gather}
где~$\mathbf{u}^{\phi}, \mathbf{u}^{\bar{\phi}}, \bm{\Sigma}_0^{\phi, \phi}, \bm{\Sigma}_0^{\bar{\phi}, \bar{\phi}}, \bm{\Sigma}_0^{\phi, \bar{\phi}}$ введены в~\eqref{eq:ap:def:1}.
\end{theorem}
\begin{proof}
TODO
\end{proof}
...
\label{eq:ap:5}
\begin{aligned}
p\bigr(\mathbf{u}^{\psi}|\mathfrak{D}\bigr) = \mathcal{N}\bigr(\mathbf{u}_{0}^{\psi}+\bm{\Sigma}_0^{\psi, \bar{\psi}}\bigr({\bm{\Sigma}_0^{\bar{\psi},\bar{\psi}}}\bigr)^{-1}\left(\mathbf{i} - \mathbf{u}_0^{\bar{\psi}}\right), \bm{\Sigma}_0^{\phi,\phi}-\bm{\Sigma}_0^{\psi, \bar{\psi}}\bigr({\bm{\Sigma}_0^{\bar{\psi}, \bar{\psi}}}\bigr)^{-1}\bm{\Sigma}_0^{\psi, \bar{\psi}}\bigr),
\end{aligned}
\end{gather}
где~$\mathbf{u}^{\psi}, \mathbf{u}^{\bar{\psi}}, \bm{\Sigma}_0^{\psi, \phi}, \bm{\Sigma}_0^{\bar{\psi}, \bar{\psi}}, \bm{\Sigma}_0^{\psi, \bar{\psi}}$ введены аналогично к~\eqref{eq:ap:def:1},
\[
...
Теорема~\ref{theorem:ap:layer} задает апостериорное распределение параметров~\eqref{eq:ap:5} после удаления слоя нейросети. Полученное распределение $p\bigr(\mathbf{u}^{\psi}|\mathfrak{D}\bigr)$ является оценкой апостериорного распределения модели без одного слоя.

\subsection{Выполнение последовательных преобразований}
Преобразования $\phi, \psi$ позволяют согласовать пространства параметров учителя~$f$ и ученика~$g$. После сопоставления параметрических моделей получаем, что параметры модели учителя и модели учителя принадлежат одному семейству~\ref{section:one:space}.
